...
define([
    ...
    $.widget('mage.editSellerProfile', {
        options: {
            ...
            confirmMessageForBackgroundImageDelete: $t('Are you sure you want to delete this background image?'),
        ...
        _create: function () {
            ...
            $(self.options.backgroundImgDeleteSelector).click(function(event){
                var dicisionapp = confirm(self.options.confirmMessageForBackgroundImageDelete);
                if (dicisionapp) {
                    var thisthis = $(this);
                    $(self.options.backgroundImgDeleteSelector).css('opacity', self.options.opacity);
                    $.ajax({
                        url: self.options.bgImgDeleteAjaxUrl,
                        type: "POST",
                        data: {},
                        dataType: 'html',
                        success:function (response) {
            ...
            $(self.options.backgroundImgDeleteSelector).mouseover(function (event) {
                $(event.target).css('width', self.options.logoImageDeleteMouseOverWidth);
            });
            ...
            $(self.options.backgroundImgDeleteSelector).mouseout(function (event) {
                $(event.target).css('width', self.options.logoImageDeleteMouseOutWidth);
            });
        },
