...
class ConsentService
{
     ...
     * @throws LocalizedException
     */
    public function deleteCustomerConsent($email, $customerId = null)
    {
        $builder = $this->criteriaBuilder;
        if ($customerId != null) {
            $builder->addFilter(ConsentInterface::CUSTOMER_ID, $customerId);
        } else {
            $builder->addFilter(ConsentInterface::GUEST_EMAIL, $email);
        }
        $collection = $this->repository->getList($builder->create());
        foreach ($collection->getItems() as $item) {
            $this->repository->deleteById($item->getConsentId());
        }
    }
     ...
     * @throws LocalizedException
     */
    public function deleteConsentWithNewsletter($email)
    {
        $builder = $this->criteriaBuilder
            ->addFilter(ConsentInterface::SOURCE, \Scommerce\Gdpr\Model\Config\Source\PrivacySource::NEWSLETTER);
        $builder->addFilter(ConsentInterface::GUEST_EMAIL, $email);
