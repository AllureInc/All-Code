 ...
 */
namespace Plenty\Item\Block\Adminhtml\Profile\Config\Form\Field\Product\Barcode;
...
class Mapping extends AbstractFieldArray
{
    ...
    protected function _prepareToRender()
    {
        $this->addColumn(
            'mage_barcode',
            ['label' => __('Magento Barcode'), 'renderer' => $this->_getMageGroupRender()]
        );

        $this->addColumn(
            'plenty_barcode',
            ['label' => __('PlentyMarkets Barcode'), 'renderer' => $this->_getPlentyGroupRender()]
        );

        $this->_addAfter = false;
        $this->_addButtonLabel = __('Add Barcode Mapping');
    }
    ...
    protected function _getMageGroupRender()
    {
        if (!$this->_mageGroupRender) {
            ...
            $this->_mageGroupRender->setClass('mage_barcode_select');
        }
    ...
    protected function _getPlentyGroupRender()
    {
        if (!$this->_plentyGroupRender) {
            ...
            $this->_plentyGroupRender->setClass('plenty_barcode_select');
        }
    ...
    {
        $optionExtraAttr = [];
        $optionExtraAttr['option_' . $this->_getMageGroupRender()->calcOptionHash($row->getData('mage_barcode'))] = 'selected="selected"';
        $optionExtraAttr['option_' . $this->_getPlentyGroupRender()->calcOptionHash($row->getData('plenty_barcode'))] = 'selected="selected"';
