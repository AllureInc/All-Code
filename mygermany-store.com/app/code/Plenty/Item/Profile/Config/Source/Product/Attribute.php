<?php
/**
 * Copyright © Soft Commerce Ltd. All rights reserved.
 * See LICENSE.txt for license details.
 */
namespace Plenty\Item\Profile\Config\Source\Product;

use Magento\Framework\Data\OptionSourceInterface;
use Magento\Catalog\Api\Data\ProductAttributeInterface;
use Magento\Catalog\Api\ProductAttributeRepositoryInterface;
use Magento\Framework\Api\SearchCriteriaBuilder;

/**
 * Class Attribute
 * @package Plenty\Item\Profile\Config\Source\Product
 */
class Attribute implements OptionSourceInterface
{
    /**
     * @var ProductAttributeRepositoryInterface
     */
    private $_productAttributeRepository;

    /**
     * @var SearchCriteriaBuilder
     */
    private $_searchCriteriaBuilder;

    /**
     * @var array
     */
    protected $_attributes;

    /**
     * Attribute constructor.
     * @param ProductAttributeRepositoryInterface $productAttributeRepository
     * @param SearchCriteriaBuilder $searchCriteriaBuilder
     */
    public function __construct(
        ProductAttributeRepositoryInterface $productAttributeRepository,
        SearchCriteriaBuilder $searchCriteriaBuilder
    ) {
        $this->_productAttributeRepository = $productAttributeRepository;
        $this->_searchCriteriaBuilder = $searchCriteriaBuilder;
    }

    /**
     * @param null $frontendInput
     * @return ProductAttributeInterface[]
     */
    public function getAttributeCollection($frontendInput = null)
    {
        if (null !== $frontendInput) {
            if (!is_array($frontendInput)) {
                $frontendInput = [$frontendInput];
            }
            foreach ($frontendInput as $value) {
                $this->_searchCriteriaBuilder->addFilter('frontend_input', $value);
            }
        }

        $criteria = $this->_searchCriteriaBuilder->create();
        return $this->_productAttributeRepository->getList($criteria)->getItems();
    }

    /**
     * @return array
     */
    public function toOptionArray()
    {
        if (null === $this->_attributes) {
            $this->_attributes = [];
            $attributes = $this->getAttributeCollection();
            /** @var ProductAttributeInterface $priceAttribute */
            foreach ($attributes as $attribute) {
                $this->_attributes[] = [
                    'value' => $attribute->getAttributeCode(),
                    'label' => $attribute->getDefaultFrontendLabel(),
                ];
            }
        }

        return $this->_attributes;
    }
}