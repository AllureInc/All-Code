...
class ManageProductRawData extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    */
    ...
    public function amzProHasVariation($proAsin, $tempProData = false)
    {
        try {
            ...
            if (empty($cmptProData['GetMatchingProductResult']['Product']['Relationships'])) {
                if ($this->helper->getAddProductFeature()) {
                    $productFeatures = $this->getProductFeatureDescription($cmptProData);
                    $tempProData['description'] = empty($productFeatures) ? $tempProData['description'] : $tempProData['description'].'<br><br><b>Product Features</b><br><br>'.$productFeatures;
    ...
    {
        if (isset($cmptProData['GetMatchingProductResult']) && isset($cmptProData['GetMatchingProductResult']['Product']['AttributeSets']['ns2ItemAttributes']['ns2Feature'])) {
            $proFeatures = $cmptProData['GetMatchingProductResult']['Product']['AttributeSets']['ns2ItemAttributes']['ns2Feature'];
            if (is_array($cmptProData['GetMatchingProductResult']['Product']['AttributeSets']['ns2ItemAttributes']['ns2Feature'])) {
        ...
        try {
            ...
            if (isset($parentAsinData['GetMatchingProductResult']['Product']['Relationships']['ns2VariationChild']) && $parentAsinData['GetMatchingProductResult']['Product']['Relationships']['ns2VariationChild']) {
                foreach ($parentAsinData as $key => $parentVal) {
                    ...
                    foreach ($parentVal['Product']['Relationships']['ns2VariationChild'] as $attributes => $attrValue) {
                        $childAsin = '';
                        if (isset($attrValue['Identifiers'])) {
                            ...
                            if (count($parentVal['Product']['Relationships']['ns2VariationChild']) > 1) {
                                $childAsin = $attrValue['MarketplaceASIN']['ASIN'];
                                if (!empty($this->getDataFromReportByAsin($childAsin))) {
                                    $superAttribute = $this->createSuperAttrMagento($parentVal['Product']['Relationships']['ns2VariationChild'], $this->attributeSetId);
                                    $hasAssociate = true;
                                    $noMoreAsin = true;
                                } else {
                                    break;
    ...
    public function getQuickCmptProData($asin, $superAttribute)
    {
        ...
        if (!empty($quickProData)) {
            if ($this->helper->getAddProductFeature()) {
                $productFeatures = $this->getProductFeatureDescription($amzAssociateData);
                $quickProData['description'] = empty($productFeatures) ? $quickProData['description'] : $quickProData['description'].'<br><br><b>Product Features</b><br><br>'.$productFeatures;
            }
            ...
            foreach ($amzAssociateData['GetMatchingProductResult']['Product']['AttributeSets'] as $proAttribute) {
                if (isset($superAttribute)) {
                    foreach ($superAttribute as $attrCode => $attrValue) {
                            ...
                            $actualAmzAttr = 'ns2'.ucfirst($amzAttr[1]);
    ...
    public function getFormattedDataByProApi($asin, $amzAssociateData)
    {
        try {
            ...
            $name = $amzAssociateProduct['AttributeSets']['ns2ItemAttributes']['ns2Title'];
            $description = $amzAssociateProduct['AttributeSets']['ns2ItemAttributes']['ns2Title'];
    ...
    public function convertPriceToBase($price)
    {
        try {
            if($ruleData = $this->helper->getPriceRuleByPrice($price)) {
                $ruleAppliedPrice = $this->helper->getPriceAfterAppliedRule($ruleData, $price, 'import');
            }
    ...
    public function getConfigProductData(
        ...
        try {
            foreach ($configAttribute as $key => $attribute) {
                $proFeaturesString = '';
                if ($this->helper->getAddProductFeature()) {
                    if (isset($attribute['ns2Feature'])) {
                        if (is_array($attribute['ns2Feature'])) {
                            $proFeaturesString =  implode('<br>', $attribute['ns2Feature']);
                        } else {
                            $proFeaturesString =  '<br>'.$attribute['ns2Feature'];
                        }
                    }
                    $description = empty($proFeaturesString) ? $attribute['ns2Title'] : $attribute['ns2Title'].'<br><br><b>Product Features</b><br><br>'.$proFeaturesString;
                } else {
                    $description = $attribute['ns2Title'];
                }
                $wholedata = [
