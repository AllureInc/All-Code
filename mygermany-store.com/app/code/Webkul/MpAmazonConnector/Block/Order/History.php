...
class History extends \Magento\Framework\View\Element\Template
{
    ...
    public function getPricebyorder($orderId)
    {
        ...
        foreach ($collection as $coll) {
            ...
            $appliedCouponAmount = $coll->getAppliedCouponAmount()*1;
            $shippingAmount = $coll->getShippingCharges()*1;
            $refundedShippingAmount = $coll->getRefundedShippingCharges()*1;
            $totalshipping = $shippingAmount - $refundedShippingAmount;
            $vendorTaxAmount = $coll->getTotalTax()*1;
            if ($coll->getActualSellerAmount() * 1) {
                $taxShippingTotal = $vendorTaxAmount + $totalshipping - $appliedCouponAmount;
                $actualSellerAmount += $coll->getActualSellerAmount() + $taxShippingTotal;
            } else {
                if ($totalshipping * 1) {
