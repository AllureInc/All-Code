...
class Order extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    /*
    \Webkul\MpAmazonConnector\Helper\Data
     */
    private $helper;
    ...
    public function createAmazonOrderAtMage($orderData)
    {
        ...
        try {
            if (!$orderData['shipping_address']['street'] || !$orderData['shipping_address']['country_id']) {
                return ['error' => 1,'msg' => __('order id ').$orderData['amazon_order_id'].__(' not contain address')];
            }
            ...
            $shipmethod = 'wk_mpamzconnectship_wk_mpamzconnectship';
            // Collect Rates and Set Shipping & Payment Method
            $this->shippingRate
                ->setCode('wk_mpamzconnectship_wk_mpamzconnectship')
                ->getPrice(1);
            ...
            $shippingAddress->setCollectShippingRates(true)
                            ...
                            ->setShippingMethod('wk_mpamzconnectship_wk_mpamzconnectship');
            $quote->getShippingAddress()->addShippingRate($this->shippingRate);

            $quote->setPaymentMethod('mpamzpayment');
            $quote->setInventoryProcessed(false);

            // Set Sales Order Payment
            $quote->getPayment()->importData(['method' => 'mpamzpayment']);
    ...
    private function createMarketplaceOrder($orderId)
    {
        try {
            ...
            foreach ($collectionSaleslist as $rowSaleslist) {
                ...
                $rowSaleslist->setOmniChannel('amazon');
                $rowSaleslist->save();
            }
            ...
            foreach ($collectionOrders as $rowOrders) {
                ...
                $rowOrders->setOmniChannel('amazon');
                $rowOrders->save();
            }
        } catch (\Exception $e) {
