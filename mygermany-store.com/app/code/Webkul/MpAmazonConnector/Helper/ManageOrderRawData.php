...
use Webkul\MpAmazonConnector\Helper\ManageProductRawData;
use Webkul\MpAmazonConnector\Model\OrderMap;
...
class ManageOrderRawData extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    */
     ...
     * @param Data $helper
     * @param \Webkul\MpAmazonConnector\Helper\Order $orderHelper
    ...
    public function __construct(
        ...
        Data $helper,
        \Webkul\MpAmazonConnector\Helper\Order $orderHelper,
    ...
    public function getFinalOrderReport($rangeData, $viaCron = false)
    {
        ...
        try {
            ...
            if (!empty($amzOrders)) {
                $notifications = $this->manageOrderData($amzOrders, $this->helper->getCustomerId(), $viaCron);
     ...
     * @return array
     */
    public function manageOrderData($amzOrders, $sellerId = false, $viaCron = false)
    {
        $items = [];
        $notifications = [];
        $errorMsg = '';
        ...
        try {
            ...
            $tempAvlImported = $this->amazonTempData
                                ->getCollection()                                                       ->addFieldToFilter('item_type', 'order')
            ...
            foreach ($amzOrders as $key => $rawOrderData) {
                ...
                if (!isset($rawOrderData['ShippingAddress']) || !isset($rawOrderData['BuyerEmail'])) {
                    continue;
                }
                ...
                if (!$viaCron) {
                    ...
                    $mapedOrder = $this->orderMapRepo
                            ->getByAmzOrderId($rawOrderData['AmazonOrderId'])
                            ->getFirstItem();
     ...
     * @return array
     */
    public function getExtraDetailsOfAmzOrder($amazonOrderId, $sellerId, $viaCron)
    {
            ...
            foreach ($amzOrderItems as $key => $amzOrder) {
                ...
                if (isset($amzOrder['ASIN'])) {
                    ...
                    $amzProCollection =  $this->productMapRepo
                            ->getByAmzProductId($amzOrder['ASIN'])->getFirstItem();
