...
define([
    ...
    "use strict";
    var popup;
    $.widget('mpamazonconnect.importscript', {
        _create: function () {
            var self = this;
            var options = this.options;
            
            // import admin amazon products
            $("body").on("click",'#mpamazonconnect-accounts-product-import', function () {
                $.ajax({
                    ...
                    success: function (amazonProd) {
                        if (amazonProd.error === 'false') {
                            if (amazonProd.data) {
                                $('#mpamazonconnect-import-product-create').removeAttr('disabled');
            ...
            $('body').on('click','#mpamazonconnect-accounts-generate-reportid', function () {
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.error_msg==false) {
                            $('#mpamazonconnect-accounts-product-import').removeAttr('disabled');
            ...
            $('body').on('click', '#mpamazonconnect-import-product-create', function (e) {
                var width = '1100';
                var height = '400';
                var scroller = 1;
    ...
    return $.mpamazonconnect.importscript;
});