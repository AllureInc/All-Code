...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
        ...
        if (!empty($data['product']['images'])) {
            ...
            foreach ($images as $key => $value) {
                ...
                if (\strpos($value, 'http') !== false) {
                    ...
                    $destinationPath = $filePath.$sku;
                    $this->_file->createDirectory($destinationPath);
                    $imageName = '/'.$profileId.'/'.$sku.'/'.baseName($value);
                    $imagePath = $this->getMediaPath().'tmp/catalog/product'.$imageName;
                    ...
                    $result = $this->fileUpload->read($value, $baseFileName);
                }
    ...
    public function processImageFromUrl($wholeData, $data, $profileId)
    {
        if (!empty($data['product']['images'])) {
            $sku = $data['product']['sku'];
            $images = array_unique($this->getArrayFromString($data['product']['images']));
            ...
            foreach ($images as $key => $value) {
                ...
                if (\strpos($value, 'http') !== false) {
                    ...
                    $tmpDirectory    = $this->getMediaDirTmpDir();
                    $this->fileUpload->checkAndCreateFolder($tmpDirectory);
                    $baseFileName = $tmpDir.'/catalog/product'.$imageName;
                    $result = $this->fileUpload->read($value, $baseFileName);
                }
                if (!empty(trim($value)) && $this->_fileDriver->isExists($imagePath)) {
                    $i++;
        ...
        foreach ($childRowArr as $key => $childRow) {
            ...
            if ($associatedProId && in_array($associatedProId, $associatedProductIds)) {
                ...
                if (!empty($assoImageData['product']['image'])) {
                    ...
                    $wholeData['configurations'][$variationMatrixIndex]['small_image'] = $assoImageData['product']['small_image'];
                    $wholeData['configurations'][$variationMatrixIndex]['thumbnail'] = $assoImageData['product']['thumbnail'];
                    $wholeData['configurations'][$variationMatrixIndex]['media_gallery'] = $assoImageData['product']['media_gallery'];
                }
                ...
                $wholeData['configurations'][$variationMatrixIndex]['quantity_and_stock_status']['qty'] = $childRowData['product']['stock'];
                $wholeData['configurations'][$variationMatrixIndex]['quantity_and_stock_status']['qty'] = $childRowData['product']['stock'];
    ...
    public function processCustomAttributeData($wholeData, $data)
    {
        foreach ($data['product'] as $code => $value) {
            ...
            $wholeData = $this->checkAttributeData($attribute, $code, $value, $notAllowedAttr, $wholeData);
        }
        return $wholeData;
    }
     ...
     * @return void
     */
    public function checkAttributeData($attribute, $code, $value, $notAllowedAttr, $wholeData)
    {
