...
class Collection extends AbstractCollection
{
    ...
    public function getMonthlyTransactionDetails($customerId, $firstDay, $lastDay)
    {
        ...
        "refundamount,refundwalletorder,usedwallet,admindebit,transfertocustomer,transfertobank,".
        "(refundwalletorder+usedwallet+admindebit+transfertocustomer+transfertobank) as totaldebit,".
        ...
        "(select COALESCE(sum(amount), 0) from wk_ws_wallet_transaction where customer_id=".$customerId.
        " AND transaction_at >= '".$firstDay."' AND transaction_at < '".$lastDay.
        "' AND sender_type=5 AND action='debit') as transfertobank,".
        "(select COALESCE(sum(amount), 0) from wk_ws_wallet_transaction where customer_id=".$customerId.
        " AND status=1 AND transaction_at >= '".$firstDay."' AND transaction_at < '".$lastDay.
