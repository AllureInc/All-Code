<?php
namespace Mangoit\SortMenu\Block\Megamenu;
...
class Topmenu extends \Smartwave\Megamenu\Block\Topmenu
{
    ...
    public function getActiveChildCategories($category)
    {
        ...
        $sub_category_tree_arr = [];
        foreach ($subcategories as $category) {
            if (!$category->getIsActive()) {
                continue;
            }
            $sub_category_tree_arr[$category->getName()] = $category;
        }

        ksort($sub_category_tree_arr);
        if (!empty($sub_category_tree_arr)) {
            foreach ($sub_category_tree_arr as $key => $category) {
                $children[] = $category;
            }
        }
    ...
    public function getSubmenuItemsHtml($children, $level = 1, $max_level = 0, $column_width=12, $menu_type = 'fullwidth', $columns = null)
    {
        ...
        if(!$max_level || ($max_level && $max_level == 0) || ($max_level && $max_level > 0 && $max_level-1 >= $level)) {
            ...
            foreach ($children as $child) {
                $sub_category_tree_arr[$child->getName()] = $child;
            }
            ...
            if (!empty($sub_category_tree_arr)) {
                foreach($sub_category_tree_arr as $key => $child) {
    ...
    public function getMegamenuHtml()
    {
        ...
        foreach ($categories as $category) {
            $category_tree_arr[$category->getName()] = $category;
            $count++;
        }

        echo "<br>";
        print_r($category_tree_arr);
        die("...33..");
        ksort($category_tree_arr);

        /*foreach($categories as $category) {*/
        foreach($category_tree_arr as $key => $value) {
            $category = $value;
