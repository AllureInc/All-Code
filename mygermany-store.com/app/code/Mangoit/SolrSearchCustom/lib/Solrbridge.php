 ...
 */
namespace Mangoit\SolrSearchCustom\lib;

use Magento\Framework\App\Bootstrap;

/*
 *  NOTE: This class is the exact copy of the Solrbridge\Search\lib\Solrbridge
 *  
 *  Method: getFilterQuery is updated for the country restrictions.
 *
 *  We could not override the class because it's a final class
 *
 */
...
final class Solrbridge
{
    ...
    public static function prepareConfig($storeId)
    {
        ...
        if (!file_exists($configFilePath)) {
            ...
            fwrite($configFile, json_encode($solrbridgeConfigData));
            fclose($configFile);
        }
    }
    ...
    public static function loadConfigData($storeId = 0)
    {
        if (null === self::$_config) {
            ...
            self::$_config = json_decode($configData, true);
        }
        return self::$_config;
    }
    ...
    public static function getConfigData($path = null)
    {
        $config = self::loadConfigData(self::$_storeId);
        if (!$path) {
            return $config;
        }
    ...
    */
    ...
    public static function getFilterQuery()
    {
        ...
        $filters['restricted_countries'] = [$countryName];
            ...
            foreach ($filters as $field => $value) {
                if (!is_array($value)) {
                    ...
                    if ($field == 'price') {
                        ...
                        $part = '';
                        foreach ($value as $val) {
                            $part .= '-filter('.self::getFilterKey($field).':"'.urldecode($val).'") OR ';
                        }
                        $part = trim(trim($part), 'OR');
                        if ( !empty($part) ) {
                            $fq .= ' AND '.trim($part).'';
    ...
    public static function searchProducts($queryText)
    {
        ...
        $arguments = array(
            ...
            'fl' => 'document_id,document_type,store_id,website_id,name_varchar,price_decimal',
            'json.nl' => 'map',
            'spellcheck' => 'true',
            'spellcheck.collate' => 'true',
            'facet' => 'true',
