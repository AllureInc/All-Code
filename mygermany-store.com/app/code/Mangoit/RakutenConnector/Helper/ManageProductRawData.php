...
class ManageProductRawData extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    */
    ...
    /**
     * @var \Mangoit\RakutenConnector\Model\Storage\DbStorage
     */
    private $dbStorage;
     ...
     * @param \Mangoit\RakutenConnector\Model\Storage\DbStorage $dbStorage
     */
    public function __construct(
        ...
        \Mangoit\RakutenConnector\Model\Storage\DbStorage $dbStorage
    ) {
    ...
    public function getFinalProductReport()
    {
        try {
            ...
            if ($this->rktnClient) {
                ...
                // $listingReport = $this->rktnClient->getReport($listReportId);

                // if (is_array($listingReport)) {
                    // $inventoryReport = $this->rktnClient->getReport($inventoryReportId);
                    $pulledProducts = $this->rktnClient->getAllRakutenProducts();

                    // $mergedFinalReport = $this->mergeInventoryAndListingReport($listingReport, $inventoryReport);

                    // $finalReport = $mergedFinalReport['report'];
                    if (!empty($pulledProducts)) {
                        $items = $this->saveDataInWellFormat($pulledProducts);
                    }
                    $response = [
                        'error' => 'false',
                        'error_msg' => false,
                ...
                //     $response = [
                //         'error' => 'true',
                //         'error_msg' => __("Product report is not ready at Rakuten Marketplace, Please try after some time.")
                //     ];
                // }
            } else {
                $response = [
                    'error' => 'true',
                    'error_msg' => __("Rakuten Client Does not Initialize.")
                ];
            }
        } catch (\Exception $e) {
    ...
    public function saveDataInWellFormat($wholeRawData)
    {
        ...
        $defaultTaxId = $this->helper->getDefaultTaxClassId();
        ...
        $alreadyMapped = $this->productMap->getCollection()->getColumnValues('rakuten_pro_id');

        $tempAvlImported =  array_merge($tempAvlImported, $alreadyMapped);

        try {
            foreach ($wholeRawData as $rktnProData) {
                if (in_array($rktnProData['product_id'], $tempAvlImported)) {
                    continue;
                }
                $wholeData = [
                    'type_id' => 'simple',
                    ...
                    'name' => utf8_encode($rktnProData['name']),
                    'description' => utf8_encode($rktnProData['description']),
                    'short_description' => ' ',
                    'sku' => $rktnProData['product_id'],
                    // 'price' => isset($rktnProData['price']) ? $rktnProData['price'] : 00.00,
                    // 'price' => $this->convertPriceToBase($rktnProData['price']),
                    // 'currency_id' =>  $this->helper->getAmazonCurrencyCode(),
                    'stock' => (isset($rktnProData['stock']) && $rktnProData['stock']) ? $rktnProData['stock'] : $defaultQty,
                    'is_in_stock' => 1,
                    'tax_class_id' => $defaultTaxId ? $defaultTaxId : 0,
                    // 'product_id' =>$rktnProData['product_id'],
                    'weight' => $defaultWeight
                ];

                $this->_mageLogger->info("### Rakuten Logger ###");
                $this->_mageLogger->info("### Rakuten Product SKU : ".$rktnProData['product_id']." ###");
                /*$this->_mageLogger->info("### Rakuten Product SKU special_price : ");
                $this->_mageLogger->info(var_dump(isset($rktnProData['special_price'])));
                $this->_mageLogger->info("### Rakuten Product SKU special_price condition 2 : ");
                $this->_mageLogger->info(var_dump(isset($rktnProData['special_price']) && $rktnProData['special_price'] > 0.00)            );*/


                if (isset($rktnProData['special_price']) && $rktnProData['special_price'] > 0) {
                     $this->_mageLogger->info("### Special Price Added Rakuten Product SKU : ".$rktnProData['product_id']."  ###");
                    $wholeData['special_price'] = $rktnProData['special_price'];
                }

                unset($rktnProData['name']);
                unset($rktnProData['description']);
                unset($rktnProData['stock']);

                $wholeData = array_merge($rktnProData, $wholeData);
                $completeWellFormedData[] = [
                    'item_type' => 'product',
                    'item_id' => $rktnProData['product_id'],
                    'item_data' => json_encode($wholeData),
                    'created_at' => $currentDate,
                    'seller_id' => $this->helper->getCustomerId(),
                    'rkt_product_id' => $rktnProData['product_id'],
                    'product_sku'    => $rktnProData['product_id']
                ];
                $items[] = $rktnProData['product_id'];
            }
            
            // print_r($items);
            // print_r($completeWellFormedData);
            // die('dfgsdfgsdfgsdfg');
            $this->insertBulkTempData($completeWellFormedData);
        } catch (\Exception $e) {
     ...
     * @return null
     */
    public function insertBulkTempData($completeWellFormedData = [])
    {
        try {
                ...
                if (count($completeWellFormedData) > $numberOfRecond) {
                    while (count($completeWellFormedData) > $indexNumber) {
                        ...
                        if (count($completeWellFormedData) > ($indexNumber+$numberOfRecond)) {
                            ...
                            $this->dbStorage->insertMultiple('marketplace_rakuten_tempdata', $slicedArray);
                    ...
                    $this->dbStorage->insertMultiple('marketplace_rakuten_tempdata', $completeWellFormedData);
                }
            }
        } catch (\Exception $e) {
            $this->logger->info('ManageProductRawData insertBulkTempData : '.$e->getMessage());
        }
    }
    ...
    private function getDetailOfAmazonProduct($amzProData)
    {
        ...
        try {
            foreach ($amzProData as $key => $value) {
                $tempAvl = $this->rakutenTempDataRepository
                            ->getByItemId(
                                'product',
                                $value['asin1']
                            )
                ...
                $isAlreadyMapped = $this->productMapRepo->getByRktProductId($value['asin1']);

                if ($isAlreadyMapped->getSize()) {
                    continue;
                }
                $wholeData = $this->getProductByRktnProId($value['asin1'], $value, false);
     ...
     * @return array|string
     */
    public function getProductByRktnProId($rktnProductId, $value = false, $viaCron = false, $isSimple = false)
    {
        // $hasVariation = $this->productHasRktnVariations($rktnProId);
        // if ($hasVariation) {
        //     $wholeData = $hasVariation;
        // } else {
            $baseCurrency = $this->storeManager->getStore()->getBaseCurrencyCode();
            $amzCurrency = $this->helper->getAmazonCurrencyCode();
            $cmptProData  = $this->rktnClient->getMatchingProduct($rktnProductId);
            $defaultQty = $this->helper->getDefaultProductQty();
            $defaultTaxId = $this->helper->getDefaultTaxClassId();
            $qty = null;
            ...
            $cmptPriceProData = $this->rktnClient->getCompetitivePricingForASIN([$rktnProductId]);
            $getMyPrice = $this->rktnClient->getMyPriceForASIN([$rktnProductId]);
        ...
        // }
            if (!$viaCron) {
                ...
                if (isset($result['product_id']) && $result['product_id']) {
                    ...
                    // save product in marketplace table
                    $this->productHelper->_saveMarketPlaceInfo($result['product_id'], $this->helper->getCustomerId());
    ...
    /**
     * check if Rakuten Product has variation or not
     *
     * @param int $productIdOnRktn
     * @return array|| null
     */
    public function productHasRktnVariations($productIdOnRktn, $tempProData = false)
    {
        try {
            $cmptProData  = $this->rktnClient->getMatchingProduct($productIdOnRktn);
            if (empty($cmptProData)) {
                throw new \Magento\Framework\Exception\LocalizedException(
                    new \Magento\Framework\Phrase('No product returned for variations check.')
                );
            }

            if ($cmptProData[0]['has_variants'] == 0) {
                // return ['is_simple' => true, 'data' => $cmptProData[0]];
                ...
                //     $tempProData['description'] = isset($productWithWholeVariation['description']) ?
                //         $productWithWholeVariation['description'] :
                //         $tempProData['description'];

                //     return ['is_simple' => true, 'data' => $tempProData];
                // } else {
                //     return ['is_simple' => false, 'data' => $productWithWholeVariation];
                // }
                $rktnProVariantOpts = $this->getRakutenVariationOptions($cmptProData[0]);
                if(isset($rktnProVariantOpts['custom_options']) && !empty($rktnProVariantOpts['custom_options'])) {
                    return ['is_simple' => false, 'data' => array_merge($tempProData, $rktnProVariantOpts)];
                } else {
                    return ['is_simple' => true, 'data' => array_merge($tempProData, $rktnProVariantOpts)];
                }
            }
        } catch (\Exception $e) {
            $this->logger->info('Helper ManageProductRawData productHasRktnVariations : '.$e->getMessage());
        }
    }

    /**
     * get product variations as products custom options.
     *
     * @param array $cmptProData
     * @return void
     */
    public function getRakutenVariationOptions($cmptProData)
    {
        if (isset($cmptProData['variants']['variant']) && $cmptProData['variants']['variant']) {
            $variants = $cmptProData['variants']['variant'];
            $customOptsValues = [];
            $count = 0;
            foreach ($variants as $variant) {
                // print_r($variant);
                // $matches  = preg_grep('/^\w+(_type)$/i', $variant);

                // $types = preg_grep('/^variant\w+(_type)$/i', array_keys($variant), 0);
                // $optStr = '';
                // // $vals = [];
                // foreach ($types as $key )
                // {
                //     $newKey = reset(explode('_', $key));
                //     if($variant[$newKey . '_value']) {
                //         $optStr .= $variant[$key] . ' (' . $variant[$newKey . '_value'] . '), ';

                //         // $vals[$newKey]['key'] = $variant[$key];
                //         // $vals[$newKey]['value'] = $variant[$newKey . '_value'];
                //     }
                // }
                $optvalue['record_id'] = $count;
                // $optvalue['title'] = rtrim($optStr, ', ');
                $optvalue['title'] = $variant['name'];
                $optvalue['price'] = $variant['price'];
                $optvalue['price_type'] = "fixed";
                $optvalue['sort_order'] = $variant['sort'];
                $optvalue['is_delete'] = '0';
                $count++;

                $customOptsValues[] = $optvalue;
                // print_r($vals);
            }
            // print_r($customOptsValues);

            $label = $cmptProData['variants']['label'];
            $customOpt = [
                [
                    "sort_order"    => 1,
                    "title"         => "Select: ".$label,
                    ...
                    "type"          => "drop_down",
                    "is_require"    => 1,
                    "values"        => $customOptsValues
                ]
            ];

            $cmptProData['custom_options'] = $customOpt;
            // print_r($customOpt);
        }
        return $cmptProData;
    }
     ...
     * @return void
     */
    public function getProductFeatureDescription($cmptProData)
    {
        if (isset($cmptProData['GetMatchingProductResult']) && isset($cmptProData['GetMatchingProductResult']['Product']['AttributeSets']['ItemAttributes']['Feature'])) {
            $proFeatures = $cmptProData['GetMatchingProductResult']['Product']['AttributeSets']['ItemAttributes']['Feature'];
            if (is_array($cmptProData['GetMatchingProductResult']['Product']['AttributeSets']['ItemAttributes']['Feature'])) {
                $proFeaturesString =  implode('<br>', $proFeatures);
            } else {
                $proFeaturesString =  '<br>'.$proFeatures;
            }
            return $proFeaturesString;
        }
    }
    ...
    //         $parentAsinData  = $this->rktnClient->getMatchingProduct($parentAsinId);
    ...
    //                 foreach ($parentVal['Product']['Relationships']['VariationChild'] as $attributes => $attrValue) {
    //                     $childAsin = '';
    //                     if (isset($attrValue['Identifiers'])) {
    //                         if (count($attrValue) > 1) {
    //                             $childAsin = $attrValue['Identifiers']['MarketplaceASIN']['ASIN'];
    //                             if (!empty($this->getDataFromReportByAsin($childAsin))) {
    //                                 $superAttribute = $this->createSuperAttrMagento($attrValue, $this->attributeSetId);
    //                                 $hasAssociate = true;
    //                             } else {
    //                                 continue;
    //                             }
    //                         }
    //                     } else {
    //                         if (count($parentVal['Product']['Relationships']['VariationChild']) > 1) {
    //                             $childAsin = $attrValue['MarketplaceASIN']['ASIN'];
    //                             if (!empty($this->getDataFromReportByAsin($childAsin))) {
    //                                 $superAttribute = $this->createSuperAttrMagento($parentVal['Product']['Relationships']['VariationChild'], $this->attributeSetId);
    //                                 $hasAssociate = true;
    //                                 $noMoreAsin = true;
    //                             } else {
    //                                 break;
    ...
    //                     }
    //                     if (!empty($childAsin)) {
    //                         $quictProData = $this->getQuickCmptProData($childAsin, $superAttribute);
    //                         if (empty($quictProData)) {
    //                             continue;
    //                         }
    //                         if (!empty($superAttribute)) {
    //                             $wholeData['supperattr'] = array_keys($superAttribute);
    //                         }
     ...
     * @return void
     */
    // public function getQuickCmptProData($asin, $superAttribute)
    // {
    //     $amzAssociateData = $this->rktnClient->getMatchingProduct($asin);
    //     if (!isset($amzAssociateData['GetMatchingProductResult'])) {
    //         return false;
    //     }
    //     $quickProData = $this->getDataFromReportByAsin($asin);
    ...
    //                         $actualAmzAttr = ucfirst($amzAttr[1]);
    ...
    {
        try {
            $getMyPriceAssociate = $this->rktnClient->getMyPriceForASIN([$asin]);

            $competitivePriceAssociate = $this->rktnClient->getCompetitivePricingForASIN([$asin]);
    ...
    public function getDataFromReportByAsin($asin)
    {
        ...
        try {
            $tmpDataByReport = $this->rakutenTempDataRepository
                                ->getByItemId(
                                    'product',
                                    $asin
                                )
    ...
    public function createSuperAttrMagento($mageSupAttrs, $attributeSetId)
    {
        ...
        try {
            ...
            foreach ($mageSupAttrs as $attrCode => $values) {
                ...
                $attributeGroupId = $this->getAttributeGroupId('Product Variation', $attributeSetId);
