...
class Product extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    */
    ...
    private function _getNewImagePath($imageUrl, $productSku)
    {
        try {
            $path = $this->filesystem->getDirectoryRead(DirectoryList::MEDIA)
                                ->getAbsolutePath().'import/RakutenConnector/';
    ...
    public function addImages($productId, $images)
    {
        try {
            ...
            foreach ($images['images'] as $image) {
                $assignImagesTo = ['small_image', 'thumbnail'];
                if($image === $images['default']){
                    $assignImagesTo = ['image', 'small_image', 'thumbnail'];
                }
                $image = trim($image);
                if ($image != '') {
     ...
     * @return int
     */
    public function getOptionValByText()
    {
        $optionVal = null;
        try {
    ...
    {
        /*$this->logger->info('----->>>> ' . json_encode($proDataReq->getParams(), JSON_PRETTY_PRINT));*/

        if (empty($this->asinOptionVal)) {
            $this->asinOptionVal = $this->getOptionValByText();
        }
        ...
        if ($this->isValidSku($sku)) {
                ...
                $wholeData['product']['price'] = isset($proData['price']) ? $proData['price'] : 0.00;
                ...
                $wholeData['product']['stock_data']['qty'] = isset($proData['stock']) ? $proData['stock'] : 0;
                if (isset($proData['special_price'])) {
                    $wholeData['product']['special_price'] = $proData['special_price'];
                    /*$wholeData['product']['special_from_date'] = date('Y-m-d', strtotime(date('Y-m-d')));

                    $dayAfterTomorrow = (new \DateTime())->add(new \DateInterval('P2D'));
                    $wholeData['product']['special_to_date'] = $dayAfterTomorrow->format('Y-m-d');*/
                }
                ...
                //     $proData['image_data'] = $this->getRktnProImage($proData['asin']);
                // }

                $proData['image_data'] = $this->getRktnProImage($proData['product_id']);

                if(isset($proData['custom_options']) && !empty($proData['custom_options'])) {
                    $wholeData['product']['options'] = $proData['custom_options'];
                }
                ...
                if ($isAssociateProduct == 1) {
                    ...
                    if (count($assocatedPro) > 0) {
                        ...
                        $proData['image_data'] = $this->getRktnProImage($assocatedPro['asin']);
                        $data = [
                            'rakuten_pro_id' => $assocatedPro['asin'],
                            'name' => $assocatedPro['name'],
                            'product_type' => 'associate',
                            'seller_id'   => $this->helper->getCustomerId(),
                            'product_sku' => $assocatedPro['sku'],
                            'mage_cat_id' => $proData['category'][0]
                        ];
                    }
                }
                try {
                    ...
                    if ($proData['type_id'] === 'configurable' && $productId && $isAssociateProduct) {
                        ...
                        if (!empty($this->helper->getCustomerId())) {
                            $this->_saveMarketPlaceInfo($productId, $this->helper->getCustomerId());
                        }
                    }
                } catch (\Execption $e) {
                    $productId = 0;
            ...
            if (!empty($productId)) {
                $result = ['error' => 0, 'product_id' => $productId, 'msg' => 'already exist'];
            } else {
                $result['error'] = 1;
                $result['msg'] = 'Skipped '.$productName.". sku '".$sku."' already exist.";
                $result['sku'] = $sku;
            }
        }
        return $result;
    ...
    public function getRktnProImage($rktnProAsin)
    {
        $imageData = [];
        try {
            $cmptProData  = $this->rktnClient->getMatchingProduct($rktnProAsin);
            // print_r($cmptProData);
            // die('asdads');
            if (isset($cmptProData[0]['images']['image'])) {
                ...
                $path = BP.'/pub/media/import/RakutenConnector/';
                if (!is_dir($path)) {
                ...
                $imageArr = [];
                $defaultImage = '';
                foreach ($cmptProData[0]['images']['image'] as $image) {
                    if($defaultImage == ''){
                        $defaultImage = (isset($image['default']) && $image['default'] == 1) ? $image['src'] : '';
                    }
                    $imageArr[] = $image['src'];
                }
                $imageData = [
    ...
    /**
     * save seller product information in marketplace table
     *
     * @param int $productId
     * @param int $sellerId
     * @return void
     */
    public function _saveMarketPlaceInfo($productId, $sellerId)
    {
        $approvalStatus = $this->helper->sellerProApprovalRequired();
        $mpProObj = $this->mpProductFactory->create();
        $mpProObj->setData(
            [
                'mageproduct_id' => $productId,
                'seller_id' => $sellerId,
                'status' => empty($approvalStatus) ? 1 : 2,
                'updated_at' => $this->localeDate->date()->format('Y-m-d H:i:s'),
                'created_at' => $this->localeDate->date()->format('Y-m-d H:i:s'),
            ]
        );
        $mpProObj->save();
    }
     ...
     * @return boolean
     */
    public function isProductMapped($asin, $sku)
    {
        $mappedColl = $this->productMap->getCollection()
                        ->addFieldToFilter('rakuten_pro_id', ['eq' =>$asin])
