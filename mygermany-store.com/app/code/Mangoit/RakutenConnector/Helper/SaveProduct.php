...
class SaveProduct
{
    ...
    public function saveProductData($proDataReq, $storeId = 0)
    {
        ...
        $loadedProduct = $this->product->load($productId);
        $loadedProduct->setStatus($wholedata['product']['status']);
        if (isset($wholedata['product']['options'])) {
            $loadedProduct->setHasOptions(1);
            $loadedProduct->setCanSaveCustomOptions(true);
        }
        $loadedProduct->save();
