...
class Export extends \Webkul\MpMassUpload\Helper\Export
{
    ...
    public function exportProducts($productType, $productIds, $allowedAttributes) {
        ...
        foreach ($productIds as $id) {
            ...
            if ($mageProductType == $productType) {
                ...
                $wholeData['type'] = $mageProduct['type_id'];
                ...
                $wholeData['shipping_price_to_mygmbh'] = $mageProduct['shipping_price_to_mygmbh'];
                $wholeData['mygmbh_shipping_product_length'] = $mageProduct['mygmbh_shipping_product_length'];
                $wholeData['mygmbh_shipping_product_width'] = $mageProduct['mygmbh_shipping_product_width'];
                $wholeData['mygmbh_shipping_product_height'] = $mageProduct['mygmbh_shipping_product_height'];
                $wholeData['product_cat_type'] = $mageProduct['product_cat_type'];
                $wholeData['fsk_product_type'] = $mageProduct['fsk_product_type'];
                $wholeData['product_note'] = $mageProduct['product_note'];
    ...
    public function prepareFileColumnRow($productType, $allowedAttributes)
    {
        ...
        $wholeData[] = 'stock';
        $wholeData[] = 'shipping_price_to_mygmbh';
        $wholeData[] = 'mygmbh_shipping_product_length';
        $wholeData[] = 'mygmbh_shipping_product_width';
        $wholeData[] = 'mygmbh_shipping_product_height';
        $wholeData[] = 'product_cat_type';
        $wholeData[] = 'fsk_product_type';
        $wholeData[] = 'product_note';
        /*Calculate product weight*/
        if ($productType != 'virtual' && $productType != 'downloadable') {
            $wholeData[] = 'weight';
        }
   ...
   //      return "";
    ...
    public function getConfigurableData(\Magento\Catalog\Model\Product $mageProduct, $wholeData)
    {
        ...
        foreach ($confAttributes as $confAttribute) {
            // echo'<pre>';print_r($confAttribute);die('hi');
        ...
        if ($configurableProductVariations) {
            ...
            foreach ($configurableProductVariations as $configurableProductVariation) {
                ...
                if (isset($productByUsedAttributes[$key])) {
                    ...
                    $associatedData['type'] = $product['type_id'];
                    ...
                    foreach ($superAttributeMainCode as $code) {
                        if (!empty($product->getData()[$code])) {
                            ...
                            if ($attribute->usesSource()) {
                                $optionText = $attribute->getSource()->getOptionText($product->getData()[$code]);
                            }
                            $attributeValues[$code] = ($optionText?$optionText:$product->getData()[$code]);
                        }
                    }
