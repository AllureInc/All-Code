...
class SalesOrderSuccessObserver extends \Webkul\Marketplace\Observer\SalesOrderSuccessObserver
{
    ...
    {
        // $observer->getOrder()->setVendorToMygermanyCost(123456);
        // $observer->getOrder()->setSccCost(786);
        ...
        $customer = $customerObj->load($customerSession->getCustomer()->getId());
        $this->_currentCustomer = $customer;

        $orderIds = $observer->getOrderIds();
        foreach ($orderIds as $lastOrderId) {
            $checkoutSession = $objectManager->create('\Magento\Checkout\Model\Session');
            $orderObj = $this->_orderRepository->get($lastOrderId);

            $sccShippingCost = $checkoutSession->getSccShippingCost();

            $vtmgCost = $checkoutSession->getVtmgShippingCost();

            $orderObj->setVendorToMygermanyCost($vtmgCost);
            $orderObj->setSccCost($sccShippingCost);
            $orderObj->save();

            //custom code 20 jul start
            $shippingMethod = $orderObj->getShippingMethod();
            $shippingAddObj = $orderObj->getShippingAddress();
            $billingAddObj =  $orderObj->getBillingAddress();
    ...
    {
        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/SalesOrderSuccessObserver.log');
