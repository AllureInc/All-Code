 ...
 */
namespace Mangoit\ShopConfigIE\Helper;
...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getBasePath($sellerId)
    {
        ...
        $basePath = $mediaPath.'mangoit/shopconfig/'.$sellerId."/";
        return $basePath;
    }
     ...
     * @return array
     */
    public function getAttributeProfiles()
    {
    ...
    public function getUploadedFileRowData($profileId = 0)
    {
        try {
            ...
            $data = $this->getProfileData($profileId);
            if ($data) {
                $uploadedFileRowData = unserialize($data->getDataRow());
            }
    ...
    */
    ...
    public function prepareProductDataIfNotSet($data, $profileType)
    {
        ...
        if ($profileType == 'downloadable' && $isDownloadableAllowed) {
            if (empty($data['product']['downloadable_link_file'])) {
                $data['product']['downloadable_link_file'] = '';
            }
            if (empty($data['product']['links_title'])) {
                $data['product']['links_title'] = '';
            }
            if (empty($data['product']['links_purchased_separately'])) {
                $data['product']['links_purchased_separately'] = '';
            }
            if (empty($data['product']['samples_title'])) {
                $data['product']['samples_title'] = '';
            }
            if (empty($data['product']['downloadable_link_file'])) {
                $data['product']['downloadable_link_file'] = '';
            }
            if (empty($data['product']['downloadable_link_price'])) {
                $data['product']['downloadable_link_price'] = '';
            }
            if (empty($data['product']['downloadable_link_title'])) {
                $data['product']['downloadable_link_title'] = '';
            }
            if (empty($data['product']['downloadable_link_type'])) {
                $data['product']['downloadable_link_type'] = '';
            }
            if (empty($data['product']['downloadable_link_sample'])) {
                $data['product']['downloadable_link_sample'] = '';
            }
            if (empty($data['product']['downloadable_link_is_sharable'])) {
                $data['product']['downloadable_link_is_sharable'] = '';
            }
            if (empty($data['product']['downloadable_link_is_unlimited'])) {
                $data['product']['downloadable_link_is_unlimited'] = '';
            }
            if (empty($data['product']['downloadable_link_number_of_downloads'])) {
                $data['product']['downloadable_link_number_of_downloads'] = '';
            }
            if (empty($data['product']['downloadable_link_sample_type'])) {
                $data['product']['downloadable_link_sample_type'] = '';
            }
        }
    ...
    public function getProductPostData($profileId, $row)
    {
        ...
        $productRowData= $this->calculateProductRowData(
            $sellerId, $profileId, $row, $profileType
        );
        ...
        if ($this->canSaveCustomAttribute()) {
            try {
                $collection = $this->_objectManager
                    ->create('Webkul\Customattribute\Model\ResourceModel\Manageattribute\Collection')
    ...
    {
        if ($this->_moduleManager->isEnabled('Webkul_Customattribute')) {
            return true;
        }
        return false;
    }
    ...
    public function validateUploadedFiles()
    {
        ...
        if ($validateCsv['extension'] == 'csv') {
            ...
            $flag = 1;
            foreach ($uploadedFileRowData['ShopConfig'] as $key => $value) {
            ...
            // print_r();
            // print_r();
            $uploadedFileRowData[0] = array_keys($jsonAry);
            $uploadedFileRowData[1] = array_values($jsonAry);
            // print_r(file_get_contents($csvFilePath));
            // die('asd');            
        } else {
            $inputFileType = PHPExcel_IOFactory::identify($csvFilePath);
    ...
    public function uploadZip($result, $fileData)
    {
        ...
        try {
            ...
            $filePath = $this->getMediaPath().'tmp/mangoit/shopconfig/'.$sellerId.'/';
            ...
            $this->copyImgFilesToSellerFolder($sellerId, $fileData, $filePath);
            $result = ['error' => false];
        } catch (\Exception $e) {
     ...
     * @return array
     */
    public function copyImgFilesToSellerFolder($sellerId, $fileData, $filePath)
    {
        $fileTempPath = $filePath.'tempfiles/';

        $imgTyps = array('product_watermark_image',
            'shop_background_img',
            'logo_pic',
            'banner_pic'
        );
        ...
        foreach ($fileData as $keyIdx => $dataVal) {
            if(in_array($keyIdx, $imgTyps)) {
                if($keyIdx == 'logo_pic' ||
                    $keyIdx == 'banner_pic' ||
                    $keyIdx == 'shop_background_img')
                {
                    $destinationPath = $this->getMediaPath().'avatar/';
                } elseif ($keyIdx == 'product_watermark_image'){
                    $destinationPath = $this->getMediaPath().'catalog/product/watermark/stores/'.$fileData['store_id'].'/';
                }

                if (!empty(trim($dataVal))) {
                    $sourcefilePath = $fileTempPath.$dataVal;
                    ...
                    if ($this->_fileDriver->isExists($sourcefilePath)) {
                        ...
                        $this->_file->copy($sourcefilePath, $destinationPath.'/'.$dataVal);
                        $optimizerChain = OptimizerChain::create();
                        $optimizerChain->optimize($destinationPath.'/'.$dataVal);
                    }
                }
            }
        }
    ...
    public function processConfigurableData($wholeData, $data, $row, $childRowArr, $uploadedFileRowData, $profileId)
    {
        ...
        foreach ($childRowArr as $key => $childRow) {
            ...
            foreach ($attributes as $attribute) {
                $attributeId = $attribute['attribute_id'];
                $code = $attribute['attribute_code']; $wholeData['product']['configurable_attributes_data'][$attributeId]['attribute_id'] = $attributeId;
    ...
    public function processAttributeData($attributeCodes)
    {
        ...
        if (strpos($attributeCodes, ',') !== false) {
            ...
            foreach ($attributeCodes as $attributeCode) {
                ...
                $attributes[] = $this->getAttributeByCode($attributeCode);
            }
        } else {
            ...
            $attributes[] = $this->getAttributeByCode($attributeCodes);
        }
    ...
    {
        $sku = $data['product']['sku'];
        $linkFiles = $this->getArrayFromString($data['product']['downloadable_link_file']);
        ...
        $linkFileIsSharable = $this->getArrayFromString(
            $data['product']['downloadable_link_is_sharable']
        );
        $linkFileIsUnlimited = $this->getArrayFromString(
            $data['product']['downloadable_link_is_unlimited']
        );
        ...
        foreach ($linkFiles as $key => $value) {
            ...
            if ($linkFileSampleType[$key] == 'file' && $this->isValidDownloadableFile($linkFileSample[$key])) {
                ...
                if ($this->_fileDriver->isExists($linkFileSamplePath)) {
                    $linkSampleInfo = pathinfo($linkFileSamplePath);
                    $linkFileSampleFile = [
    ...
    {
        $sellerId = $this->getCustomerId();
        $sellerGroupHelper = $this->_objectManager->create('Webkul\MpSellerGroup\Helper\Data');
        ...
        if (count($getSellerTypeGroup) && $getSellerTypeGroup->getNoOfProducts()) {
     ...
     * @return bool
     */
    public function checkProductAllowedStatus($rowCount)
    {
        $sellerId = $this->getCustomerId();
        ...
        if (isset($groupExpireStatus['expire'])? $groupExpireStatus['expire'] :false) {
            return false;
        } elseif (count($getSellerTypeGroup)) {
