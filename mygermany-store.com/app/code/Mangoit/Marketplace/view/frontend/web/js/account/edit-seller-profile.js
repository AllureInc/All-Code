...
define([
    ...
    $.widget('mage.editSellerProfile', {
        options: {
            ...
            confirmMessageForBgImgDelete: $t('Are you sure you want to delete this background Image?'),
        ...
        _create: function () {
            ...
            $(self.options.backgroundImgDeleteSelector).click(function (event) {
                console.log('consoleeedddd');
                var dicisionapp = confirm(self.options.confirmMessageForBgImgDelete);
                if (dicisionapp) {
                    var thisthis = $(this);
                    //$(self.options.logoSelector).css('opacity', self.options.opacity);
                    $.ajax({
