 ...
 */
class MassDisapprove extends \Webkul\Marketplace\Controller\Adminhtml\Product\MassDisapprove
{
    /**
    ...
    public function execute()
    {
        ...
        foreach ($collection as $item) {
            ...
            $emailTemplateVariables['myvar3'] = number_format($productModel->getPrice()).' EUR';
            ...
            // Product FAQs disapproval- Start
                ...
                if ($currentFaqDetails->count() > 0) {
                    foreach ($currentFaqDetails as $faqValue) {
                        $faqValue->setIsActive(0);
                        $faqValue->save();
                    }
                }
            // Product FAQs disapproval- End
        }
        $this->setDisApproveStatusOnChild($productIds, $magentoProductModel, $sellerProductModel);
    ...
    public function setDisApproveStatusOnChild($productIds, $magentoProductModel, $sellerProductModel)
    {
        foreach ($productIds as $key => $id) {
                ...
                if (!empty($idArray)) {
                    foreach ($idArray as $child) {
                        if ($sellerProductModel->load($child->getId(), 'mageproduct_id')) {
                            $sellerProductModel->setStatus(2);
