...
use Webkul\Marketplace\Controller\Product\Builder as Builder;
...
class SaveProduct extends \Webkul\Marketplace\Controller\Product\SaveProduct
{
    ...
    public function __construct(
        ...
        parent::__construct($eventManager, $date, $dateFilter, $catalogProductTypeManager, $variationHandler, $productTypeConfigurable, $productRepositoryInterface, $productLinkFactory, $linkResolver, $productLinks, $jsHelper, $filesystem, $marketplaceProductBuilder, $marketplaceHelperData, $registry,  $productCopier, $messageManager, $mpProductFactory, $mpProductCollectionFactory, $stockRegistryProvider, $stockConfiguration, $jsonHelper, $customerModel, $categoryModel, $mpEmailHelper, $productCustomOption, $linkBuilder, $linkFactory, $categoryLinkManagement, $sampleBuilder, $sampleFactory, $objectManager, $storeManager, $resource, $seller, $session);
    }
    ...
    public function saveProductData($sellerId, $wholedata)
    {
        ...
        $sellerData = $this->_seller->load($customerData->getId(),'seller_id');
        if (!empty($wholedata['id'])) {
            ...
            $urlKeyExist = $this->createUrlKey($wholedata['product']['name'],$wholedata['product']['sku']);
            if ($urlKeyExist) {
                ...
                $urlKey = $wholedata['product']['name'].'-'.strtolower($sellerData->getShopUrl());

                $wholedata['product']['url_key'] = $urlKey;
                $isUnique = $this->checkUrlKeyDuplicates($sku, $urlKey, $storeId);
                if (!$isUnique) {
                    $wholedata['product']['url_key'] = $urlKey.rand(10,100);
                }
            }
            $mageProductId = '';
            $editFlag = 0;
        ...
        // Sensitive Attribute work - Start
        $sensitiveArray  = [];
        ...
        $arributesToSave = [];
        foreach ($sensitiveArray as $sensiKey => $sensiValue) {
            if (strpos($sensiKey, 'mis_2_41_35_comment') != true) {
                $explodedKey = explode('mis_2_41_35_sensitive_', $sensiKey);
                $wholedata['product'][$explodedKey[1]] = $sensiValue;
            }
        }
        // Sensitive Attribute work - End
        if ($sellerData->getTrustworthy()) {
            $status1 = SellerProduct::STATUS_ENABLED;
        } else {
            if ($mageProductId) {
        ...
        */
        ...
        $catalogProduct->setTypeId($wholedata['type']);
        ...
        if(isset($wholedata['product']['wk_marketplace_availability'])) {
            $catalogProduct->setData('wk_marketplace_availability', $wholedata['product']['wk_marketplace_availability']);
            $catalogProduct->getResource()->saveAttribute($catalogProduct, 'wk_marketplace_availability');
        }
    ...
    private function sendProductMail($data, $sellerId, $editFlag = null)
    {
        ...
        $message_one = __('I would like to inform you that recently');
        $message_for_new = __('has added a new product in the store.');
        $message_for_update = __('has updated a product in the store.');
        ...
        if ($editFlag == null) {
            ...
            $emailTempVariables['myvar4'] = $message_one.' '.$sellerName.' '.$message_for_new;
            ...
            $emailTempVariables['myvar4'] = $message_one.' '.$sellerName.' '.$message_for_update;
        }
        $senderInfo = [
            'name' => $sellerName,
            'email' => $sellerEmail,
        ...
        $emailTempVariables['myvar5'] = $sellerName;
