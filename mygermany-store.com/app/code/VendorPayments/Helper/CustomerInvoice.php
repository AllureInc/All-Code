...
class CustomerInvoice extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getCustomerCSVContent($dateFrom = '', $dateTo = ''){
        // print_r(get_class_methods($this->mageInvoiceModel));
        ...
        foreach ($mageInvoiceColl as $invoice) {
            $invoicePrefixNum = $this->getInvoicePrefixNum($invoice->getIncrementId(), '19');
            ...
            // var_dump($order->getAddressById($order->getBillingAddressId())->debug());
            ...
            $invoiceData['Prefix Invoice Number'] = $invoicePrefixNum;
            $invoiceData['Column D'] = $this->getColumnDeeValue($invoicePrefixNum);
            $invoiceData['Order-Name'] = '';
            ...
            $invoiceData['VAT %'] = '19%'; // TODO dynamic
            $invoiceData['Turnover Total'] = $order->getBaseGrandTotal();
            $invoiceData['VAT Amount'] = $order->getBaseTaxAmount();
            $invoiceData['Turnover Net'] = $order->getBaseSubtotal();
            $invoiceData['Item incl VAT'] = $order->getBaseSubtotalInclTax();
            $invoiceData['Shippin incl VAT'] = $order->getBaseShippingInclTax();
            $invoiceData['Discount/Surchagre incl VAT'] = $order->getBaseDiscountAmount();
    ...
    public function getVendorCSVContent($dateFrom = '', $dateTo = ''){
        ...
        foreach ($collection as $invoice) {
            $invcData = $salesListItems->addFieldToFilter('total_amount', ['neq' => 0])
                    ->addFieldToFilter('order_item_id',  array('in' => $invoice->getOrderIds()));
            ...
            $totalInclVat = number_format(
                    ((array_sum($totalAmountArray) - array_sum($totalFeeArray)) * 19) / 100,
                    2,
                    '.',
                    ''
                );
            ...
            $sellerData = $this->marketplaceHelper->getSellerDataBySellerId($invoice->getSellerId())->getData();
            $sellerData = isset($sellerData[0]) ? $sellerData[0] : $sellerData;

            // print_r($sellerData['name']);
            // die;
            $invoiceData = [];
            $invoiceData['Date'] = $invoice->getCreatedAt();
            $invoiceData['Invoice / Credit Number'] = $invoice->getInvoiceNumber();
            $invoiceData['Vendor costs'] = 00;
            $invoiceData['Revenue'] = 00;
            $invoiceData['Order-Name'] = '';
            $invoiceData['Order-Nummer'] = $invoice->getOrderIds();
            $invoiceData['Vendor Name'] = isset($sellerData['name']) ? $sellerData['name'] : 'NA';
            $invoiceData['Vendor / Creditor Number'] = $invoice->getSellerId();
            ...
            $invoiceData['Fees net'] = array_sum($totalFeeArray);
            $invoiceData['Fees Total (cumulated) incl. VAT'] = $totalFee;
    ...
    public function prepareFileHeader($headerTyp = 'customer')
    {
        ...
        if($headerTyp == 'customer'){
            ...
            $wholeData[] = 'Prefix Invoice Number';
            $wholeData[] = 'Column D';
    ...
    /**
     * Get Column D invoice prefix number by Prefix Number
     *
     * @param string $prefixNumer
     *
     * @return string
     */
    public function getColumnDeeValue($prefixNumer = 0)
    {
        /*
         * 41201 => 41503
         * 43154 => 44006
