requirejs(['algoliaBundle', 'Magento_Catalog/js/price-utils'], function (algoliaBundle, priceUtils) {
	algoliaBundle.$(function ($) {
		...
		/* Function for adding pagination widget in algolia search*/
		...
		const { items, hasNoResults, refine, widgetParams } = renderOptions;
			...
			const customHitsPerPage = algoliaBundle.instantsearch.connectors.connectHitsPerPage(
			  renderHitsPerPage
			);
			...
			search.addWidgets([
			  customHitsPerPage({
			    ...
			    items: [
			      { label: 'Show 12 per page',  value: 12, default: true },
			      { label: 'Show 24 per page', value: 24 },
			      { label: 'Show 36 per page', value: 36 },
			    ],
			  })
			]);
		...
		/* Adding the below line for changing text of relevance */
		algoliaConfig.translations.relevance = "Sort by position";
		...
		var customAttributeFacet = {
			categories: function (facet, templates) {
				...
				hierarchicalMenuParams.panelOptions = {
					templates: {
						header: '<div class="name filter-options-title active">' + (facet.label ? facet.label : facet.attribute) + '</div>',
					}
				};
		...
		window.getFacetWidget = function (facet, templates) {
			var panelOptions = {
				templates: {
					header: '<div class="name filter-options-title active">'
