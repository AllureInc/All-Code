 ...
 * Last Modified: 2020-04-09T12:25:03+00:00
...
class Email extends AbstractClass
{
    ...
    public function saveFiles($fileArray)
    {
        ...
        if (version_compare($utilsHelper->getMagentoVersion(), '2.3.3', '>=')) {
            ...
            $messageData['from'] = [$this->objectManager->create('Magento\Framework\Mail\AddressFactory')->create(['email' => $senderEmail, 'name' => '=?utf-8?B?' . base64_encode($senderFromName) . '?='])];
            foreach (explode(",", $this->getDestination()->getEmailRecipient()) as $email) {
                $messageData['to'][] = $this->objectManager->create('Magento\Framework\Mail\AddressFactory')->create(['email' => $email, 'name' => '=?utf-8?B?' . base64_encode($email) . '?=']);
            }
            $messageData['subject'] = '=?utf-8?B?' . base64_encode($this->replaceVariables($this->getDestination()->getEmailSubject(), implode("\n\n", $bodyFiles))) . '?=';
