 ...
 * 
 * Class for adding more product attributes in Algolia
 */
namespace Cnnb\AlgoliaFix\Helper\Entity;
...
class ProductHelper extends \Algolia\AlgoliaSearch\Helper\Entity\ProductHelper
{
    ...
    public function __construct(
        ...
        parent::__construct($eavConfig, $configHelper, $algoliaHelper, $logger, $storeManager, $eventManager, $visibility, $stockHelper, $stockRegistry, $currencyManager, $categoryHelper, $priceManager, $productType, $productCollectionFactory, $groupCollection, $imageHelper);
        ...
        $this->_newAndSaleProducts = $this->getNewAndSaleProducts();
    }
    ...
    public function getNewAndSaleProducts()
    {
        return $this->_algoliaFixHelper->getNewAndSaleProductsData();
    }
    ...
    public function getObject(Product $product)
    {
        ...
        $this->_logger->info(" ========= Custom Data ========");
        $this->_logger->info(print_r($customData, true));
        $this->_logger->info(" ========= Custom Data ========");

        if ($product->getMetaTitle() && $product->getMetaKeyword() && $product->getMetaDescription()) {
            $this->_logger->info(' # Meta Title: '.$product->getMetaTitle());
            $this->_logger->info(' # Meta Keyword: '.$product->getMetaKeyword());
            $this->_logger->info(' # Meta Description: '.$product->getMetaDescription());
        }

        if (isset($this->_newAndSaleProducts['new']) && !empty($this->_newAndSaleProducts['new'])) {
            if (in_array($product->getId(), $this->_newAndSaleProducts['new']) ) {
                $customData['new'] = true;
                $this->_logger->info(' # NEW PRODUCT | '.$product->getId().' | '.$product->getName());
            }
        }

        if (isset($this->_newAndSaleProducts['sale']) && !empty($this->_newAndSaleProducts['sale'])) {
            if (in_array($product->getId(), $this->_newAndSaleProducts['sale']) ) {
                $customData['sale'] = true;
                $this->_logger->info(' # SALE PRODUCT | '.$product->getId().' | '.$product->getName());
            }
        }
    ...
    private function addAdditionalAttributes($customData, $additionalAttributes, Product $product, $subProducts)
    {
        foreach ($additionalAttributes as $attribute) {
            ...
            $this->_logger->info('--- Attribute Name: '.$attributeName);
            $this->_logger->info(' -- CustomData --');
            /*$this->_logger->info(print_r($customData, true));*/
            ...
            if ($product->getData('sku') == 'MH07' && $product->getData('sku') == '24-MG03') {
                $this->_logger->info('------ Product Data ------');
                $this->_logger->info(print_r($product->getData(), true));
            }

            $value = $product->getData($attributeName);

            $this->_logger->info(' -- value !== null checking ---');
            if ($value !== null) {
                $this->_logger->info(' -- value !== null checked ---');
                ...
                if (!in_array($attributeName, $this->attributesToIndexAsArray, true)) {
                    $this->_logger->info(' -- in array failed ---');
                    continue;
                }
            }
        ...
        /*$this->_logger->info(' -- CustomData | Final --');
        $this->_logger->info(print_r($customData, true));*/
