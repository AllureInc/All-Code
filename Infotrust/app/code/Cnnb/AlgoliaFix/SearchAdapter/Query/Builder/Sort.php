...
namespace Cnnb\AlgoliaFix\SearchAdapter\Query\Builder;
...
class Sort
{
    ...
    public function getSort(RequestInterface $request)
    {
         ...
         * Checked the $item['field'] should not be null
         */
        if (!method_exists($request, 'getSort')) {
            return $sorts;
        }
        foreach ($request->getSort() as $item) {
            if (in_array($item['field'], $this->skippedFields) || $item['field'] == null) {
                continue;
            }
