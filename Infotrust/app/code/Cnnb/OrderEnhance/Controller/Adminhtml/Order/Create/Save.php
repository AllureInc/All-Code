 ...
 *
 */
namespace Cnnb\OrderEnhance\Controller\Adminhtml\Order\Create;
...
class Save extends \Magento\Sales\Controller\Adminhtml\Order\Create\Save
{
    ...
    public function execute()
    {
        ...
        $this->helper = ObjectManager::getInstance()->create(CnnbHelper::class);

        try {
            $this->logger->info("Class: ". __CLASS__.", Function: " . __FUNCTION__);
            ...
            if ($order->getId() && $this->helper->isModuleEnable()) {
                $this->logger->info('--- Create Order Comment Is Adding ---');
                $this->helper->addCommentInOrder($order->getId(), $this->helper->getOrderReorderComment());
                $this->logger->info('--- Create Order Comment Has Been Added ---');
            }
