...
class Processor
{
    ...
    public function __construct(
        \Kerastase\AdminActivity\Model\Config $config,
        \Magento\Backend\Model\Auth\Session $authSession,
        \Kerastase\AdminActivity\Model\Handler $handler,
        \Magento\Framework\HTTP\PhpEnvironment\RemoteAddress $remoteAddress,
        \Kerastase\AdminActivity\Model\ActivityFactory $activityFactory,
        \Kerastase\AdminActivity\Model\ActivityLogDetailFactory $activityDetailFactory,
        ...
        \Kerastase\AdminActivity\Model\Activity\Status $status,
        \Kerastase\AdminActivity\Model\Handler\PostDispatch $postDispatch
    ) {
        $this->config = $config;
    ...
    {
        if (\Kerastase\AdminActivity\Helper\Data::isWildCardModel($model)) {
    ...
    public function addLog($activity, $logData, $model)
    {
        ...
        $this->activityLogs[] = [
            \Kerastase\AdminActivity\Model\Activity::class => $activity,
            \Kerastase\AdminActivity\Model\ActivityLog::class => $logData,
            \Kerastase\AdminActivity\Model\ActivityLogDetail::class => $logDetail
        ];
    }
        ...
        try {
            if (!empty($this->activityLogs)) {
                foreach ($this->activityLogs as $model) {
                    $activity = $model[\Kerastase\AdminActivity\Model\Activity::class];
                    $activity->save();
                    $activityId = $activity->getId();

                    if (isset($model[\Kerastase\AdminActivity\Model\ActivityLog::class])) {
                        $logData = $model[\Kerastase\AdminActivity\Model\ActivityLog::class];
                        if ($logData) {
                    ...
                    if (isset($model[\Kerastase\AdminActivity\Model\ActivityLogDetail::class])) {
                        $detail = $model[\Kerastase\AdminActivity\Model\ActivityLogDetail::class];
